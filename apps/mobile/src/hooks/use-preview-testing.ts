...
export function usePreviewTesting(options: TestingOptions) {
  ...
  }, [options.active?.bundleUrl, options.webVisible, options.enabled])
  ...
  useEffect(() => {
    ...
    for (const event of options.events) {
      ...
      // Hidden routes and previous revisions must not consume the visible
      // Preview's one-shot request. Retry retained events when content mounts.
      if (!options.active?.opened || options.active.revision !== probe.revision) continue
      if (options.active.target === 'web' && !options.webVisible && probe.kind !== 'close') continue
      ...
      void (async () => {
        ...
        try {
          ...
          if (claimed.command.kind === 'close') {
            ...
            webReplies.current.clear()
            await selected.closePreview()
            result = { timestamp: Date.now(), closed: true }
          } else if (!active.opened) {
            throw new Error('Preview startup is still in progress. Wait for its startup result before testing.')
          } else if (active.target === 'native') {
          ...
          if (claimed.command.kind !== 'close' && observedGeneration !== generation.current) throw new Error('Preview closed or changed during inspection. Open it and inspect again.')
  ...
  }, [options.events, options.projectId, options.enabled, options.active?.revision, options.active?.opened, options.webVisible])

...
export function runtimeRequestTimeoutMs(method: MobileHostMethod): number {
  if (method === 'release.export' || method === 'source.export' || method === 'source.import.commit' || method === 'host.suspend' || method === 'agent.run' || method === 'agent.resume' || method === 'agent.cancel' || method === 'project.clone' || method === 'project.import.githubSnapshot' || method === 'git.share.publish') return 10 * 60_000

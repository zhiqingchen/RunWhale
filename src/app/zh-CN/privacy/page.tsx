...
const title = "哪里跑隐私政策";
const description =
  "哪里跑隐私政策：了解本地数据、第三方 AI 共享、数据删除以及网站托管。";
const socialImage = {
...
export default function ChinesePrivacyPage() {
  return (
    <ContentPage
      ...
      eyebrow="隐私"
      title="隐私政策"
      summary="本政策适用于哪里跑应用及本网站，说明哪些数据留在设备上、哪些数据会共享给你选择的服务，以及你的隐私选择。"
    ...
    >
      <section>
        ...
        <p>应用无需哪里跑账号，也不向哪里跑发送使用统计或广告标识。</p>
      </section>

      <section>
        <h2>应用处理的信息</h2>
        <p>哪里跑会将项目、Git 历史、智能体会话、附件、偏好设置、项目缓存和生成的项目数据保存在设备上的应用本地容器中。模型服务商的 API 密钥和设备的 Git SSH 私钥使用操作系统的安全存储保存。</p>
        <p>哪里跑不会将你的项目或智能体会话副本上传至哪里跑服务器。</p>
      </section>

      <section>
        <h2>发送给你所选服务的信息</h2>
        <p>AI 功能会将请求内容及回答所需的上下文发送给所选模型服务。数据接收方、数据类别、授权选择和服务商条款详见下一节。</p>
      ...
      <section>
        <h2>第三方 AI 服务</h2>
        <p>发送 AI 请求内容前，RunWhale 会显示所选服务商和 API 地址，并征求你的明确同意。授权针对该接收方；更换服务商或地址需要重新同意。拒绝后不会发送该 AI 请求。</p>
        <p>经你同意，Agent 会按照你的选择，将提示词、对话历史、相关源码和文件内容、工具结果，以及作为上下文使用的图片或 Preview 截图发送给 Anthropic（Claude API）、DeepSeek、Google（Gemini API）或 OpenAI。图像生成和网页搜索也可能通过该服务商发送请求内容。接收服务会收到用于认证的 API 密钥，以及 IP 地址等连接信息。请求用于生成你要求的回复、代码、图片或操作，可能包含文件或图片中的个人信息。</p>
        <p>使用自定义 API 地址时，请求会发送给该地址的运营方，后者可能继续转发给其他服务。仅凭模型或服务商名称无法确定该运营方。授权前请了解其身份、隐私政策及后续转发方式。</p>
      ...
      <section>
        ...
        <p>在 Preview 中运行的代码可能连接自己的服务，并处理你输入或允许其访问的信息，包括照片、相机、麦克风或位置。运行前请检查项目源码及其连接的服务。在系统设置中管理设备权限。被加入智能体请求的 Preview 内容，会按照上述数据共享选择发送给所选 AI 服务。</p>
      </section>
      ...
      <section>
        ...
        <p>信息会与你指示应用使用的服务及上述网站托管服务商共享；或在遵守法律、保护权利与安全、调查滥用行为时按要求共享。</p>
      </section>
      ...
      <section>
        ...
        <p>你可在“设置 → 关于 → 隐私政策 → AI 数据共享”撤回 AI 同意。撤回会停止正在运行的智能体，并阻止进一步的模型请求，直到你再次同意。撤回不会移除第三方已接收的信息。应用内包含本隐私政策的离线副本。</p>

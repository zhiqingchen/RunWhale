...
export async function createMobileHarness(options: MobileHarnessOptions): Promise<MobileHarness> {
  ...
  ctx.systemPrompt.section({ name: 'runwhale-sdk', order: 1255, text: 'RunWhale includes @runwhale/sdk. In React projects, import { useLanguage } from \'@runwhale/sdk\' and call it inside components to receive { language, loading, error }. Language is en, zh-CN, es, fr, or ja and follows RunWhale settings automatically. Keep each supported locale in a peer project-local JSON dictionary and load all locales through the same translator. Use English only as a fallback for unsupported locales or missing messages. Handle initial loading and connection errors, and do not add a separate language preference or read Studio storage. The SDK is already available in Preview; no native build or installation is needed.' })
